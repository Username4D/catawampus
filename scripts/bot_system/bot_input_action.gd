...
func _init(_init_type: input_types = input_types.PRESS, _init_action_name: String = "ui_up", _init_time_stamp: float = 0.1, _init_state: Resource = PlayerState.new(0,false, 0, Vector2.ZERO)) -> void:
	...
	if _init_state: state = _init_state
